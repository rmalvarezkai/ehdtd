...
def main(argv): # pylint: disable=unused-argument
    ...
    # symbol = 'MSTAR/USDT' # pylint: disable=unused-variable
    ...
    end_time = end_time - ((86400 * 5) + 0)
    # end_time = min(end_time, int(time.time()))
    start_time = end_time - (BingxEhdtdAuxClass.get_delta_time_from_interval(interval) * (limit+1)) # pylint: disable=unused-variable
    ...
    __aux_test_class = BingxEhdtdAuxClass
    ...
    # __test_data = __aux_test_class.get_kline_data(symbol, interval, start_time=start_time,\
